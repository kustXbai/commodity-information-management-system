<template>
  <div>
    <el-card class="box-card">
      <!-- 卡片顶部添加品牌按钮 下面本来要加v-has="btn.Trademark.add" 暂时去掉 -->
      <el-button type="primary" size="default" icon="Plus" @click="addTrademark" >添加品牌</el-button>
      <!-- 表格组件：用于展示已有得平台数据 -->
      <!-- table
        ---border:可以设置表格纵向是否有边框
        table-column  表示的是边框的列
        ---label:某一个列表
        ---width:设置这列宽度
        ---align:设置这一列对齐方式 -->
      ...
      <!--分页器组件
            pagination
                ...
                layout:可设置分页器留个子组件的布局调整。
          -->
      <el-pagination @size-change="sizeChange" @current-change="getHasTradeMark" :pager-count="9"
        v-model:current-page="pageNo" :page-sizes="[3, 5, 7, 9]" v-model:page-size="limit" :background="true"
        layout=" prev,pager,next,jumper,->,sizes,total " :total="total">
      </el-pagination>
    </el-card>
    <!-- 对话框组件：在添加品牌和修改已有品牌业务时候使用的结构 -->
    <!-- 
...
//组件挂载完毕 钩子---发一次请求，获取第一页，一页三个已有品牌数据。
onMounted(() => {
  getHasTradeMark()
})
...
const confirm = async () => {
  //在你发请求之前,要对于整个表单进行校验
  //调用这个方法进行全部表单相校验,如果校验全部通过，再执行后面的语句   （所以用await）
