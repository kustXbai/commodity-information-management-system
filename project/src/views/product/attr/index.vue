...
const addAttr = () => {
    ...
    Object.assign(attrParams, {
        ...
        categoryId: categoryStore.c3Id,//点击添加属性按钮时候收集三级分类的ID
        categoryLevel: 3,//代表的是三级分类
    })
    //切换为添加与修改属性的结构
    scene.value = 1;
...
const updateAttr = (row: Attr) => {
    ...
    //ES6->Object.assign进行对象的合并    JSON.stringify(row)用这个进行深拷贝，修改表格内的数据不会影响外部数据。
...
//路由组件销毁（比如跳转到其他路由的时候）的时候，把仓库分类相关的数据清空
onBeforeUnmount(() => {
    //清空仓库的数据
    categoryStore.$reset();
})
